...
# 	def predict(self, x):
# 		if self.hparams['SMA']:
# 			#print("SMA is executed")
# 			return self.network_sma(x)
# 		else:
# 			print("SMA not executed")
...
def get_ensemble_test_acc(exp_path, nenv, dataset_name, data_dir, hparams, force=False, var=False, file_path=None):
	...
	for env in range(nenv):
		dataset = vars(dataloader)[dataset_name](data_dir, [env], hparams)
		...
		Algorithm_all = []
		#print("model id is : ",valid_model_id)
		for model_path in valid_model_id[env]:
			#print(model_path)
			Algorithm_ = EoA(dataset.input_shape, hparams, dataset.num_classes)
