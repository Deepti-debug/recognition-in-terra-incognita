...
transval_path = '~/content/segregation/trans_val/'

train_full_directory = train_path+'full/'
cistest_full_directory = transtest_path+'full/'
cisval_full_directory = transval_path+'full/'
...
for epoch in range(20):
    ...
    if val_loss_epoch <= best_loss:
        best_loss = val_loss_epoch
        best_model = model
        print('[{:2d},  save, {}]'.format(epoch, '/home2/vishwakarma/trans_train_inception2.pt'))
        if torch.cuda.device_count() > 1:    
            torch.save(model.module.state_dict(), '/home2/vishwakarma/trans_train_inception2.pt')
        else:
            torch.save(model.state_dict(), '/home2/vishwakarma/trans_train_inception.pt')
